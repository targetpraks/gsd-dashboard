...
The Infinity Group measurement dashboard — **INF GSD Limited Edition**. Built from the INF GSD Limited Edition measurement contract (`inf-gsd-limited-edition.json`) — the single canonical source for how the portfolio is measured.
...
- **Metric Library** — all metrics defined once, searchable.
- **Alert Center** — red-flip alerts per the INF GSD alerting rules (critical/warning).
...
`inf-gsd-limited-edition.json` is the canonical measurement contract. Any AI, agent or system reading, writing or reporting on portfolio metrics reads this and nothing else.
